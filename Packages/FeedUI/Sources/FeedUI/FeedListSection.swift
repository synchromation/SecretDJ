	...
	var body: some View {
		...
		LazyVStack(spacing: Spacing.small) {
			ForEach(items) { item in
				// Never AnyView (lazy-sections skill) — exhaustive over
				// every FeedCellProps case so a new payload variant fails to
				// compile here instead of silently rendering nothing.
				// .controlTile is unreachable in a list-kind section (mood
				// tiles are matrixControlLarge-only, which maps to grid).
