...
struct ColorRoleTests {
	...
	@Test func `background is a near-white surface in light and a deep near-black in dark`() {
		let token = Theme.ColorRole.background.token

		#expect(token.light.relativeLuminance > 0.9)
		#expect(token.dark.relativeLuminance < 0.01)
	}
	...
	@Test func `accent stays a distinguishable violet in both appearances`() {
