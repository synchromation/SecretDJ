...
| [observability](observability/SKILL.md) | Logging, analytics, and crash breadcrumbs | Pipeline-only emission (no vendor SDKs in features), typed per-feature analytics enums, explicit breadcrumbs (screens/interactions/network), diagnostic level routing policy, privacy rules for events, composition of destinations, RecordingDestination test pattern |
