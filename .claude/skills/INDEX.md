...
| Skill | Scope | Owns |
|---|---|---|
| [ios-architecture](ios-architecture/SKILL.md) | Structure of features and placement of code | Feature folder anatomy, multi-target placement (single-app features in the app's Features/, shared features and infrastructure in local packages), @Observable model rules, dependency-injection seams (protocol + production + in-memory fake), composition root, separation of concerns at app/type/function level, scope discipline (task-bounded code, seams per real dependency, boundary-only validation), Swift 6 concurrency stance, feature definition of done |
...
| [lazy-sections](lazy-sections/SKILL.md) | Vertically scrolling, backend-driven feeds of heterogeneous sections | Single-outer-lazy-container feed structure, closed SectionKind enum with exhaustive switch (no AnyView, unknown kinds dropped), immutable pre-computed cell values with stable ids, scroll-performance rules (stable scaled dimensions, carousel containment, no shadows/materials in scroll content, event-driven programmatic scrolling, pinning off by default), feed-specific accessibility adaptations |
...
| [localization](localization/SKILL.md) | All user-facing copy and its translation | String Catalog mechanism (literal extraction, translator comments, plural/gender variation, placeholders), the five-language set (en source + es/fr/de/nl), tone of voice and its per-language adaptation sheets, glossary, load-bearing phrases, no-logic-on-visible-strings rule, server-text rule, copy definition of done |
...
| [checkpoints](checkpoints/SKILL.md) | How in-progress work is made visible | Commit-and-push-per-checkpoint rule (checkpoint = verifying state, never commit red), small frequent commits, NOTES.md append-only journal and its delta in every commit, journal-vs-skill boundary |
