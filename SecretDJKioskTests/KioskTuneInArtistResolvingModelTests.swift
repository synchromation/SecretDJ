...
/// ``KioskTuneInArtistResolvingModel`` — the single-song-artist lookup the
/// kiosk's TuneIn screen needs when it's reached by name rather than song id
/// (a search result artist row with exactly one song — S7.6). Mirrors the
/// consumer's own `TuneInArtistResolvingModelTests`.
@MainActor
enum KioskTuneInArtistResolvingModelTests {
	struct `Starting up` {
...
) -> KioskTuneInArtistResolvingModel {
	KioskTuneInArtistResolvingModel(artistName: artistName, musicSearching: musicSearching)
}
