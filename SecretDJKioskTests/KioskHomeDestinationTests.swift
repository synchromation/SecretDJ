...
/// tap outcomes actually push a screen. PLAN.md S7.4/S7.5: a song tap goes
/// through TuneIn to request it (LEGACY.md "the kiosk's whole write path");
/// a jukebox tile drills into that jukebox's own song grid; everything else
/// (search, artist lists, moderation) has no kiosk screen and is dropped.
enum KioskHomeDestinationTests {
	struct `Mapping an outcome` {
		...
		@Test func `drops launchSearch, out of scope until S7-6`() {
			#expect(KioskHomeDestination(outcome: .launchSearch) == nil)
		}
